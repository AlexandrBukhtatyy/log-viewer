...
# (План работ) Отдельный концепт «Роль B: in-app BYOK агент»

> Транзиентная секция — описывает работу, которую надо выполнить после одобрения. На этапе исполнения её
> содержимое выносится в новый файл, а сама секция из этого документа удаляется (чтобы концепт остался чистым).

## Context

Пришёл сторонний ответ (Gemini) с предложением сделать в приложении агента в духе **Roo Code**: чат + LLM +
agentic loop **внутри** PWA, без бэкенда, на пользовательском ключе (BYOK). Это **другая роль**, чем наш текущий
концепт Web MCP (где мы — _сервер инструментов_, а агент и модель живут у хоста/Chrome). Чтобы не смешивать роли
и не раздувать действующий док, заводим **отдельный концепт-док** про Роль B и связываем его с этим перекрёстными
ссылками. Обе роли строятся на **одном реестре tools** (`src/mcp/tools/`) и **одном egress-редакторе**
(`src/mcp/privacy/`) — это и есть архитектурная скрепа.

## Deliverable

1. **Новый файл** `docs/plans/in-app-byok-agent.md` (концепт-исследование, статус «фантазия»).
2. **Кросс-линк** в этом документе: расширить буллет «Обратный сценарий (мы как клиент…)» в разделе 1 ссылкой
   на новый док.
3. Удалить эту транзиентную секцию из `web-effervescent-biscuit.md`.
4. (По правилу автокоммита) закоммитить оба файла одним `docs(plans): …`. Push — только по явной команде.
5. **GitHub — структура Epic + sub-issues + зависимость:**
   - Создать **epic-issue** «AI-агент в log-viewer (Web MCP + in-app)» с кратким описанием двух ролей и
     ссылками на оба концепт-дока.
   - Сделать **#32** (Роль A) и **новый issue Роли B** — _sub-issues_ этого эпика.
   - Пометить Роль B как **blocked by #32** (зависимость: B нельзя начать без общего реестра tools + redactor’а
     из Роли A).
   - Добавить epic и issue Роли B в проект **Log Viewer**, выставить **Status = Planned**, **Area = ai**.
   - В тело issue Роли B вписать ссылку на `docs/plans/in-app-byok-agent.md`.
6. **Priority** (`low|med|high`): **A (#32) = high**, **B = med**; epic — `high`.
7. **Инструменты:** sub-issues/зависимости управляются `gh` с релиза CLI 2026-06-10 (флаги `--blocked-by`,
   sub-issue команды). Если установленный `gh` старее — фолбэк на GraphQL API (`addSubIssue`,
   `addIssueDependency`). На исполнении проверить `gh --version`.

## Структура нового дока `in-app-byok-agent.md`

- **Context / две роли.** Таблица A (Web MCP server) vs B (in-app BYOK agent): кто держит модель, нужен ли ключ,
  кто гоняет loop, где живёт UI. Явная ссылка назад на `web-effervescent-biscuit.md`.
- **Пользовательские сценарии.** Встроенный чат над логами пользователя: «почему сервис падал в 03:00?» — агент
  сам прогоняет `lv_set_filter`/`lv_group_by`/`lv_get_histogram` через tools и резюмирует; «собери таймлайн
  инцидента по trace abc123»; «объясни этот стектрейс». UI: чат слева, существующий viewer справа — агент рулит
  той же выборкой, что видит человек.
- **Архитектура (client-side agentic loop).** Диаграмма (Mermaid) цикла: LLM → tool_call → `execute` (экшен
  `ViewStore`) → **redactor** → результат → LLM, до завершения задачи. Нативный structured tool-calling
  провайдера (НЕ ручной XML-парсинг).
- **Переиспользование реестра tools.** Один `defineTool` питает оба канала: адаптер `toModelContextTool()` для
  `navigator.modelContext` (Роль A) и `toAiSdkTool()` для Vercel AI SDK `tool()` (Роль B). Схемы — из тех же
  zod-контрактов и `FieldDescriptor`.
- **Приватность — обязательна.** В Роли B данные уходят напрямую в сторонний LLM (OpenAI/Gemini/Anthropic),
  поэтому egress-редактор из раздела 4 действующего дока здесь не опция, а требование. Ссылка на раздел 4.
- **BYOK и безопасность ключа.** Хранение ключа — не в голом `localStorage` (риск XSS-эксфильтрации), а
  аккуратнее (IndexedDB + предупреждение, не светить в логах); CORS из браузера (у Anthropic — opt-in заголовок
  `anthropic-dangerous-direct-browser-access`); выбор провайдера; **дефолт на свежие Claude (Opus 4.8)**.
- **Что осознанно отвергаем из ответа Gemini (с обоснованием).** WebContainers (мы не пишем и не запускаем код;
  требует COOP/COEP и лицензии StackBlitz) · виртуальная FS `write_file/read_file/edit_file` (наш tool-surface
  доменный, а не файловый CRUD) · ручной парсинг `<call_tool>` XML (есть нативный tool-calling). Плюс мелкие
  неточности Gemini: пакет — `@ai-sdk/google` (`createGoogleGenerativeAI`), а `aiState` — это RSC-вариант,
  требующий сервера.
- **Новые модули.** Набросок `src/agent/`: `chat/` (UI чата), `loop/` (оркестрация tool-calling), `provider/`
  (BYOK-провайдеры), `key-store/` (хранение ключа). Переиспользует `src/mcp/tools/` и `src/mcp/privacy/`.
- **Открытые вопросы / триггеры ADR** и **Verification** (юнит-тест loop с мок-провайдером и мок-`ViewStore`;
  ручной прогон BYOK-сценария на фикстуре `large.jsonl`; проверка, что в исходящих запросах к LLM нет
  нередактированных PII).

## Verification (этого плана работ)

- Новый файл существует, открывается, Mermaid-диаграмма валидна (без `{}`/`[]`/кавычек в подписях — урок
  прошлой итерации).
- В `web-effervescent-biscuit.md` появилась рабочая ссылка на новый док; транзиентная секция удалена.
- `pnpm format:check` проходит (оба файла отформатированы prettier'ом).
- В GitHub: epic создан; #32 и issue Роли B — его sub-issues; B помечен `blocked by #32`; обе (и epic) в
  проекте со Status=Planned, Area=ai; Priority A=high, B=med, epic=high. Проверка через
  `gh issue view <B> --json parent,blockedBy` и доску проекта.
