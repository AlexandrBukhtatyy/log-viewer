...
- Каждый batch до сих пор имеет один `path` (`directory-adapter` уже эмитит frame-ы с `path`). На каждый новый `path`:
  ...
  3. Если нет — старый auto-detect на первом батче этого `path` (sample из первых N строк *этого файла*).
...
- `src/core/types/log-resource.ts`:
  ...
  export interface LogResource {
    ...
    readonly meta: { accent?: string; alias?: string; defaultColumns?: LvColumnPref[]; defaultFilter?: Partial<LogFilter> };
  }
  ```
