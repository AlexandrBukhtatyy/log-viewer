...
**Field-aware подсказки (гибрид).** Свободный поиск матчит только текст — `key:value`
не парсится. Поэтому подсказки полей различаются по категории:

- **Динамические** JSON-поля → значение вставляется как **текстовый термин** (группа Values):
  для JSON-логов значение реально присутствует в тексте строки.
- **Системные** (`@level` из histogram-`levelCounts`; `@source.name/kind` из списка источников)
  и **логические** (`~`-поля, distinct-значения **лениво** через `getGroupCounts` —
  [use-field-value-suggestions.ts](../../src/hooks/use-field-value-suggestions.ts), кэш по
  сигнатуре фильтра, cap по полю) → группа **Fields** с пунктами `key = value`. Принятие
  добавляет **структурный `fieldFilter`** (`{key, op:'=', value}`), а не текст — корректно
  для значений, живущих в колонке/`JSON_EXTRACT`, а не в тексте строки. Чип виден в поповере
  Filters и применяется в SQL. Построитель остаётся чистым: контейнер передаёт
  `structuredValues` (`{key,label,value,count}`), фильтрация по токену — в
