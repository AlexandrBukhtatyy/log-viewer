...
- Bad: гейтить prune-on-hydrate через `sources.length > 0` — компромисс. Если пользователь _внешне_ стёр все источники (через DevTools/IDB), но `lv:workspace` остался непустым, ghost-id'ы лежат в localStorage до первого мутирующего действия. Зрительно безопасно — `canPrune`-фильтр в `tabs` useMemo прячет их.
...
- [src/hooks/**tests**/use-workspace.test.ts](../../src/hooks/__tests__/use-workspace.test.ts) — unit-тесты на partialize / merge / actions
