...
- **UI/UX-конвенции** — в [docs/ui-conventions.md](docs/ui-conventions.md). Сверяйся перед вёрсткой UI: подписанные поля форм — только через компонент `<LvFormField>` ([src/ui/components/common/LvFormField.tsx](src/ui/components/common/LvFormField.tsx)); модалки — лейбл слева, узкие панели — сверху.
