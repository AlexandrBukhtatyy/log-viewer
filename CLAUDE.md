...
Любые временные файлы, которые ты создаёшь по ходу отладки — Playwright-скриншоты, дампы JSON-ответов, выгрузки логов воркера, ad-hoc заметки — клади **только** в [.tmp/](.tmp/). Каталог уже в `.gitignore`. В корне репозитория ничего не оставляй: для подстраховки `/*.png /*.jpg /*.jpeg` тоже игнорятся, но это safety-net, а не место хранения.
